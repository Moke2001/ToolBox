...
from State.StateTenpy.StateTenpy import StateTenpy
from Format.ModelFormat.ModelFormat import ModelFormat
from Algorithm.Interface.InterfaceTenpy.GetModelTenpy import get_model_tenpy
from Algorithm.Interface.InterfaceTenpy.GetLatticeTenpy import get_lattice_tenpy
...
def ground_state_solver_tenpy(model_format)->tuple[float, StateTenpy]:
    ...
    dmrg_params = {
        ...
        'max_E_err': 1.e-10,
        'trunc_params': {
            'chi_max': model_format.get_site_number() * 5,
            'svd_min': 1.e-10
        },
        'combine': True
    }
    lattice_tenpy=get_lattice_tenpy(model_format)
    psi=MPS.from_product_state(lattice_tenpy.mps_sites(), [0] * model_format.get_site_number())
    model_tenpy, C_list, N_list = get_model_tenpy(model_format)
    info = dmrg.run(psi, model_tenpy, dmrg_params)
    psi_final=StateTenpy(psi,model_format.get_dimension_array())
