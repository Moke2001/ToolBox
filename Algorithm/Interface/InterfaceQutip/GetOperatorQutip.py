import numpy as np
from Algorithm.Interface.InterfaceQutip.FunctionQutip import FunctionQutip
...
from Algorithm.Interface.InterfaceQutip.GetLocalOperatorQutip import get_local_operator_qutip
    ...
    ##  SECTION：OverallMultiTermFormat作用量-------------------------------------------------------
        ...
        for index_tuple_iteration, site_iteration in np.ndenumerate(model.get_dimension_array()[..., 0]):
            ...
            for i in range(len(index_tuple_list_temp)):
                if not all(index_tuple_list_temp[i][it] < model.get_cell_period_list()[it] for it in range(len(index_tuple_list_temp[i]))):
                    flag = False
                    break
