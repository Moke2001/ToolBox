...
from Physics.QuantumComputation.Encoder.Group.PauliGroup import PauliGroup
...
class PauliCode:
    ...
    def get_rank(self):
        if self.rank is None:
            matrix=BitString.get_matrix(self.generator_vector)
    ...
    def push(self,checker,*args):
        ...
        if self.number_checker==0:
            ...
            self.generator_vector=np.empty(self.number_qubit, dtype=PauliGroup)
            ...
            self.generator_vector[self.number_checker-1]=checker_now
            self.weight=None
            self.rank=None
        ...
        for i in range(1, self.number_qubit):
            ...
            for error_position in error_position_list:
                for error_combination in error_combination_list:
                    ...
                    error_temp.define(1, pauli_vector_temp)
            ...
            for j in range(flag, len(syndrome_list)):
                ...
                if all(x == 0 for x in syndrome_list):
                    ...
                    if BitString.get_rank(error_and_stabilizer) == self.get_rank() + 1:
                        self.weight = weight
                        return weight
                for k in range(j):
