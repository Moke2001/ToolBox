...
class PauliCode:
    ...
    def get_rank(self):
        if self.rank is None:
            matrix=BitString.get_matrix(PauliGroup.get_bitstring(self.generator_vector))
    ...
    #%%  USER：第一定义函数
    """
    input.checker：PauliGroup or list of int or np.array of int对象，稳定子生成元的格式
    input.*args：int对象，稳定子生成元的系数，默认为1
    influence：如果是第一次推送，对PauliCode做初始化；如果不是第一次推送，则检查qubit数目是否匹配，匹配的话推送进去
    """
    def push(self,checker,*args):
        ##  规范化操作
        if isinstance(checker, PauliGroup):
            checker_now=checker.copy()
            number_qubit_now = checker.number_qubit
        elif isinstance(checker, list) or isinstance(checker, np.ndarray):
            checker_now=PauliGroup()
            if len(args)==0:
                checker_now.define(checker,1)
            elif len(args)==1:
                checker_now.define(checker,args[0])
            number_qubit_now = checker_now.number_qubit
        else:
        ...
        if self.number_checker==0:
            ...
            self.number_qubit=number_qubit_now
            self.generator_vector=np.empty(1, dtype=PauliGroup)
            self.generator_vector[0]=checker_now
            self.weight=None
            self.rank=None
        else:
            assert number_qubit_now == self.number_qubit,'输入的稳定子量子比特数目不符合要求'
            self.number_checker=self.number_checker+1
            self.generator_vector=np.append(self.generator_vector,checker_now)
            self.weight=None
            self.rank=None
    ...
    def get_weight(self):
        ...
        syndrome_list = []
        error_list = []
        for i in range(1, self.number_qubit):
            """
            选取所有可能的错误位
            例如如果有3个qubits，权重为2，那么为：[(0,1),(0，2),(1,2)]
            """
            error_position_list=itertools.combinations(range(self.number_qubit), i)
            ...
            for error_position in error_position_list:
                for error_combination in error_combination_list:
                    ...
                    pauli_vector_temp=np.zeros_like(s_0.data_vector, dtype=int)
                    ...
                    error_temp=PauliGroup()
                    error_temp.define(pauli_vector_temp,1)
            ...
            for j in range(flag, len(syndrome_list)):
                ...
                if all(x == 0 for x in syndrome_list):
                    ...
                    if BitString.get_rank(PauliGroup.get_bitstring(error_and_stabilizer)) == self.get_rank() + 1:
                        self.weight = weight
                        return weight
                for k in range(j):
    ...
    """
    def get_ratio(self):
        return (self.number_qubit - len(self.generator_vector)) / self.number_qubit
    ...
    def get_logical_operator(self):
        ##  构造校验矩阵
        check_matrix = np.zeros((self.number_qubit * 6, self.number_qubit * 2), dtype=int)
        for i in range(self.generator_vector.shape[0]):
            for j in range(self.number_qubit * 2):
        ...
        zeros_vector = GF(np.zeros(self.number_qubit * 2, dtype=int))
