...
from Framework.QutipBox.ModelQutip.Function import Function
from Framework.QutipBox.LatticeQutip.LatticeQutip import LatticeQutip
...
class ModelQutip(ModelFormat,LatticeQutip):
    ...
    def initial_check(self):
        if self.H is not None and self.H_list!=[] and self.C_list!=[] and self.N_list!=[]:
            return True
        else:
            return False
        ...
        for term_temp in self.terms:
            ...
            ##  遍历局域相互作用量
                ...
                for index_tuple, site_iteration in np.ndenumerate(self.site_array):
                    if index_tuple[-1] == term_temp.get_position():
            ...
            ##  遍历两体相互作用量
                ...
                inner_index_0,inner_index_1,vector=term_temp.get_position()
                for index_tuple, site_iteration in np.ndenumerate(self.site_array[...,0]):
                    index_tuple_0=()
                    index_tuple_1=()
                    for k in range(len(index_tuple)):
                        index_tuple_0=index_tuple_0+(index_tuple[k],)
                        index_tuple_1=index_tuple_1+(index_tuple[k]+vector[k],)
                    if all(index_tuple_1[it]<self.cell_period_list[it] for it in range(len(index_tuple_1))):
                        index_tuple_0=index_tuple_0+(inner_index_0,)
                        index_tuple_1=index_tuple_1+(inner_index_1,)
                        operator_0 = self.get_operator_qutip(op_0, index_tuple_0)
                        operator_1 = self.get_operator_qutip(op_1, index_tuple_1)
                        operator = operator+operator_0 * operator_1
