...
def main(N,J_1,J_2,J_z,g_z,disorder,t_list,psi):
    ##  构造系统晶格
    ...
    for i in range(N):
        for j in range(2):
            term=OnsiteTermFormat('disorder','hamiltonian',(i,j),'sigmaz',np.random.uniform(g_z-disorder,g_z+disorder,1)[0])
            model.push(term)


    ##  构造可观测量
    M_start = OnsiteTermFormat('obserbe', 'observe', (0, 0), 'sigmaz', 1)
    M_end=OnsiteTermFormat('obserbe', 'observe', (N-1, 1), 'sigmaz', 1)
    M_mid=OnsiteTermFormat('obserbe', 'observe', (int(N/2), 1), 'sigmaz', 1)
