...
class MultiTermFormat(TermFormat):
    ...
    def copy(self):
        if self.time:
            return MultiTermFormat(self.label, self.position_list, self.op_list, self.function, self.function_params)
        else:
            return MultiTermFormat(self.label, self.position_list, self.op_list, self.strength)